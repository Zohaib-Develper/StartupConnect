...
function InvestorDashboard() {
  ...
  const myInvestments = investments.filter(
    (inv) => inv.investorId === user?.id,
  );
  const totalInvested = myInvestments.reduce((acc, inv) => acc + inv.amount, 0);
  const uniqueProjects = new Set(myInvestments.map((inv) => inv.projectId))
    .size;
    ...
    >
      <div className="container">
        <div className="d-flex flex-column flex-md-row justify-content-between align-items-start align-items-md-center mb-4 animate-fade-in-up">
          ...
          <Link
            to="/projects"
            className="btn btn-accent mt-3 mt-md-0"
        ...
        <div className="row g-4 mb-4">
          {[
            ...
            },
            {
              ...
              label: "Fully Funded",
              value: portfolioProjects.filter((p) => p.status === "funded")
                .length,
              color: "var(--success)",
            },
          ].map((stat, idx) => (
            <div className="col-12 col-sm-6 col-lg-3" key={idx}>
              <div
                className="glass-card p-3 p-md-4 animate-fade-in-up"
              ...
              {portfolioProjects.length === 0 ? (
                <div className="text-center py-4">
                  ...
                  <p style={{ color: "var(--text-muted)" }}>
                    Explore startups and make your first investment
                  </p>
